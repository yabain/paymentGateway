...
export class DetailsComponent implements OnInit, OnDestroy {
  ...
  getPayerList() {
    this.gettingPayer = true;
    this.servicesService
      .getPayersList(this.serviceData._id)
      .subscribe((data: any) => {
        this.subscribers = data;
        this.gettingPayer = false;
      });
  }
  ...
  getId() {
    ...
    this.isDashboardRoute = this.url.includes('services');
  ...
  getServiceDataById(planId: string) {
    this.servicesService.getMyServicesData(planId).subscribe((data: any) => {
      if (!data) {
  ...
  isAuthor(serviceData: any = this.serviceData, user: any = this.currentUser) {
    return user?._id.toString() === serviceData?.author._id.toString() ? true : false;
  }
  ...
  getCurrentUser() {
    ...
    this.userService.getCurrentUser().then((user: any) => {
      ...
      if (
        ...
        this.planForm(this.serviceData);
      }
      this.isAuthor(this.serviceData);
      if (user.isAdmin) this.isAdmin = true;
      this.currency = this.serviceData.currency;
      if (this.isAuthor(this.serviceData) || this.isAdmin) {
        this.getPayerList();
      }
  ...
  savePicture() {
    ...
    this.servicesService.updateServicePicture(this.serviceData._id, this.selectedImage)
      .subscribe
      ({
        next: (serviceData) => {
 ...
 */
  showPreview(event: any) {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (e: any) => {
        this.serviceData.imageUrl = e.target.result;
      };
  ...
  cancel() {
    if (this.serviceData) {
      this.serviceData.imageUrl = this.memoryImage;
      this.isChangePicture = false;
    }
  }
  ...
  calculateTaxesAmount(): number {
    this.estimation = this.quantity * this.serviceData.price;
  ...
  setTransactionData() {
    this.transactionData = {
      ...
      senderCurrency: this.serviceData.currency,

      raisonForTransfer: this.paymentService.transactionType.SERVICE,
      userId: this.currentUser._id,

      receiverName: this.showName(this.serviceData.author),
      receiverEmail: this.serviceData.author.email,
      receiverContact: this.serviceData.author.phone,
      serviceId: this.serviceData._id,
      receiverCurrency: this.serviceData.currency,
      quantity: this.quantity,
      receiverAmount: this.getCleanAmount(),

      receiverId: this.serviceData.author._id,
