...
export class SystemService {
  ...
  async getRacineToshare(): Promise<string> {
    ...
    if (systemData?.value) {
      ...
      const resp = await this.getSystemData().toPromise();
  ...
  generateUniqueId(type) {
    return this.currentDate.toISOString().split('.')[0] + '-' + type + '-' + this.getRandomPaires(2);
  }
  ...
  getSystemData(): Observable<any | undefined> {
    return this.apiService.get(`system`)
    .pipe(
      map((res: any) => {
        if (res) {
    ...
    return this.getSystemData().pipe(
      map((response: any) => {
        ...
        if (response[0].appVersion !== version) {
          ...
          if (this.plateforme === "web") {
            this.translate.get("update.updateMessage").subscribe((res: string) => {
  ...
  async getStaticData() {
    ...
    let countriesData: any = await this.storage.getStorage(environment.countries_data);
    if (countriesData.value && countriesData.value.length >= 3) {
      countriesData = JSON.parse(countriesData.value);
    ...
    let citiesData: any = await this.storage.getStorage(environment.cities_data);
    if (citiesData.value && citiesData.value.length >= 2) {
      citiesData = JSON.parse(citiesData.value);
